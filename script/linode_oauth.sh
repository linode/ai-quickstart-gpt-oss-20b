...
open_browser() {
    ...
    if [[ "$OSTYPE" == "darwin"* ]]; then
        ...
        # WSL (Windows Subsystem for Linux) - detect via /proc/version
        # Use powershell.exe with proper escaping for URLs with special chars
        # Note: explorer.exe opens URLs as file paths, cmd.exe /c start fails with & and =
        if command -v powershell.exe &> /dev/null; then
            ...
            # Use cmd.exe with proper quoting (wrap entire URL in double quotes)
...
<body>
    ...
    <script>
        ...
        function attemptClose() {
            ...
            // Method 2: Try to close via opener (if window was opened by script)
            if (window.opener) {
                window.opener = null;
                window.close();
            }

            // Method 3: Try alternative close methods
            setTimeout(function() {
...
extract_oauth_token() {
    ...
    if [ "$SILENT_MODE" = false ]; then
        echo "" >&2
        echo -e "${GREEN}Opening brower. Please login with your Linode credintial." >&2
        echo "" >&2
        sleep 3
        echo -e "If the browser doesn't open automatically, visit the following URL manually:${NC}" >&2
        echo "" >&2
        echo "$oauth_url" >&2
        echo "" >&2
    fi
